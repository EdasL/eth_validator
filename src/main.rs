...
type Shard = u64; // Shard number
type Root = i32; // a Merkle root
type Slot = u64; // a slot number
...
pub fn get_shard_transition_fields(
    ...
    for slot in offset_slots.iter() {
        ...
        if shard_block_slots.contains(slot) {
            shard_block = &shard_blocks[shard_block_slots.iter().position(|x| x == slot).unwrap()];
